// pages/store_operation_category_update/store_operation_category_update.js
...
Page({
  ...
  delete_category:function(e){
    let that = this
    db.collection('category').doc(that.data.id).remove({
  ...
  submit:function(e){
    ...
    if(e.detail.value.name!==""&&that.data.img.length!==0&&that.data.fileID!==""){
      db.collection('category').doc(that.data.id).update({
        data:{
  ...
  onLoad: function (options) {
    ...
    db.collection('category').doc(options.id).get({
      success:function(res){
        console.log('信息获取成功',res)
        that.setData({
          name:res.data.name,
          fileID:res.data.file_src,
          img:res.data.src,
        })
      },fail:function(res){
